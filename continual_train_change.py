...
import time
from lreid_dataset.datasets.get_data_loaders import *
...
def main_worker(args):
    ...
    all_set = training_set

    testing_only_set = ['market1501']

    all_train_sets,all_test_only_sets = build_data_loaders(args,training_set,testing_only_set) # 返回[dataset,num_classes,train_loader, test_loader, init_loader, name]
...
def train_dataset(args, all_train_sets, all_test_only_sets, set_index, model, out_channel, writer,logger_res=None):
    ...
    current_pids = {pid for _,pid,_ in dataset.train}
    ...
    add_num = len(train_dataset.global_pid_set)
    
    if set_index>0 and num_new_classes > 0: # 不是第一个数据集
        '''store the old model''' 
        ...
        old_num_classes = org_classifier_params.size(0)
        model.module.classifier = nn.Linear(out_channel, add_num, bias=False) # 新分类器，分类数为旧类别数+新类别数
        model.module.classifier.weight.data[:old_num_classes].copy_(org_classifier_params) # 复制旧分类器前add_sum行参数到新分类器前面部分
        model.cuda()  # 重新放到GPU上
        # Initialize classifer with class centers    
        class_centers = initial_classifier(model, init_loader) # 每个类中心
        new_class_centers = torch.stack([class_centers[pid] for pid in new_pids],dim=0)
        model.module.classifier.weight.data[old_num_classes:].copy_(new_class_centers) # 为新类别的分类器提供合理的初始值
        model.cuda() # 重新放到GPU上
    ...
    Stones = [15, 30,45] if name == 'market1501_cam0' else args.milestones # msmt17数据集训练时间长，调整学习率的epoch也相应变大
    ...
    for epoch in range(0, Epochs):
        ...
        trainer.train(epoch, train_loader,  optimizer, training_phase=set_index + 1,
                      train_iters=1, add_num=0, old_model=old_model,proto_type=proto_type
                      ) # 训练一个epoch  train_iters=len(train_loader)
...
def test_model(model, all_train_sets, all_test_sets, set_index,  logger_res=None):
    ...
    dataset, num_classes, train_loader, test_loader, init_loader, name = all_test_sets[0] # 取出数据集 类别数 训练集加载器 测试集加载器 初始化集加载器 名称
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--batch-size', type=int, default=32) # 原本是128
    ...
    parser.add_argument('--epochs0', type=int, default=1) # 原本是30
    parser.add_argument('--epochs', type=int, default=1) #  原本是30
    ...
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                            default=osp.join(working_dir, '../Market-1501'))
    ...
    parser.add_argument('--n_sampling', default=1, type=int, help="number of sampling by Gaussian distribution") # 原本是6
